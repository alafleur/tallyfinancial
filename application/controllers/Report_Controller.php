		...
		{			
			...
			if(!empty($data['arUsers']))
			{
				...
				foreach($data['arUsers'] as $user)
				{
					...
					$fWeek1ExpenseAverage = format_number($arLastMonthCalculations[0]['fWeek1ExpenseAverage']);
					$fWeek2ExpenseAverage = format_number($arLastMonthCalculations[0]['fWeek2ExpenseAverage']);
					$fWeek3ExpenseAverage = format_number($arLastMonthCalculations[0]['fWeek3ExpenseAverage']);
					$fWeek4ExpenseAverage = format_number($arLastMonthCalculations[0]['fWeek4ExpenseAverage']);
					$fWeek5ExpenseAverage = format_number($arLastMonthCalculations[0]['fWeek5ExpenseAverage']);

					$fAverageIncomeMonthly = format_number($arLastMonthCalculations[0]['fAverageIncomeMonthly']);
					$fAverageIncomeWeekly = format_number($arLastMonthCalculations[0]['fAverageIncomeWeekly']);
					
					$fMaxExpenseAverage = format_number($arLastMonthCalculations[0]['fMaxExpenseAverage']);
					$fMaxExpenseCover = format_number($arLastMonthCalculations[0]['fMaxExpenseCover']);
					
					$fTotalSaving = format_number($fTotalSaving);
					$fTotalTransfers = format_number($fTotalTransfers);
					$fTallyBalance = format_number($fTallyBalance);
					$fAvgSaving = format_number($iTotalTransactions > 0 ? ($fTotalSaving/$iTotalTransactions) : 0);
	
					$csv_string .= "{$user['id']}, {$user['szFirstName']} {$user['szLastName']}, {$user['szEmail']}, {$user['szMobilePhone']},{$user['szFinicityInstitution']},{$user['szFinicityAccountTransitNumber']},{$user['szFinicityAccountNumber']},".format_number($fChequingbalance).",{$user['szInstitution']},{$user['szTransitNumber']},{$user['szAccountNumber']}," . ((int)$user['iSavingAccountVerified'] == 1 ? 'Yes' : 'No') . "," . ((int)$user['iSignupStep'] == 5 ? 'Yes' : 'No') . ",$fTallyBalance,$fTotalTransfers,$fTotalSaving,$fAvgSaving,$iTotalTransactions,".format_number($user['fAbsoluteMinBalance']).",".format_number($user['fSurplusDeficitRate']).",".format_number($user['fIfDeficitRate']).",$dtFirstTransaction,$tFirstTransaction,$fWeek1ExpenseAverage,$fWeek2ExpenseAverage,$fWeek3ExpenseAverage,$fWeek4ExpenseAverage,$fWeek5ExpenseAverage,$fAverageIncomeMonthly,$fAverageIncomeWeekly,$fMaxExpenseAverage,$fMaxExpenseCover\r\n";				
				}

		...
		if($is_user_exists)
		{
			...
			if(!empty($_POST['arMsg']))
			{
				...
				if(!$is_error)
				{
					...
					$replace_ary['CHEQUINGBALANCE'] = format_number($data['fChequingAccountBalance']);
					$replace_ary['TOTALSAVINGS'] = format_number($data['fTotalSaving']);
					$replace_ary['TALLYBALANCE'] = format_number($data['fTallyBalance']);
					$replace_ary['EXTERNALBALANCE'] = format_number($data['fTotalTransfers']);
					$replace_ary['MOSTRECENT'] = format_number($data['fMostRecentTransaction']);
				...
				else
				{
					...
					if(trim($_POST['arSearch']['szType']) == "export")
					{
						...
						if(!empty($data['arTransactions']))
						{
							...
							{
								$csv_string .= "{$transaction['id']}, {$transaction['idAccount']}, ".format_number($transaction['fAmount'], false).", {$transaction['szDescription']}, {$transaction['szCategory']}, {$transaction['szStatus']}, ".date("m/d/Y", strtotime($transaction['dtDate']))."\r\n";
							}
						}
						ob_end_clean();
						header("Pragma: ");
			...
			else
			{
				if($arg2 == "reject" && (int)$arg3 > 0)
				{
					if($this->User_Model->rejectCustomerSavingsTransfer($arg3))
					{
						...
						sendMessege($this->User_Model->szMobilePhone, "Your withdrawal request for $" . format_number($arg4) . " has been rejected. Please contact us for more details.");
					}
				}
						...
						else
						{
							if((int)$_POST['p_approve'] > 0)
							{
								...
								if($this->User_Model->approveCustomerSavingsTransfer($p_id, $p_date, $p_type))
								{
									...
									// send message to admin
									sendMessege($this->User_Model->szMobilePhone, "Your withdrawal request for $" . format_number($fTransferAmount) . " has been approved.");
								}
							}
							else
							{

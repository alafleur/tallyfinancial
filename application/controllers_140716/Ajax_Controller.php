	...
	public function index()
	{
		...
		else if($p_func == "LOGIN_WITH_INSTITUTION" || $p_func == "LOGIN_WITH_INSTITUTION_MFA")
		{
				...
				if(!empty($arInstitutionDetails))
				{
					...
					if($response)
					{
						...
						else
						{
							...
							{
								echo "MFA|||{$arInstitutionDetails[0]['szName']}|||";
								$mfa_for_message = "Activating your account, please answer once more...";
					...
					else if($this->Finicity_Model->isLoginMFA)
					{
						...
						if($is_activation)
							...
							$mfa_for_message = "Adding your accounts, please answer...";
						$show_mfa_form = true;
						$mfa_session = $this->Finicity_Model->szLoginMFASession;
					}
					else if($this->Finicity_Model->isGetStatementMFA)
			...
			<form name="frmBankLoginMFA" id="frmBankLoginMFA" action="" method="post">
				<h6 class="color-blue"><?=$mfa_for_message?></h6>
				...
				<?php }?>
				<?php if($is_activation || $activation_mfa){?>

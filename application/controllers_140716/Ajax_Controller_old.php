...
class Ajax_Controller extends CI_Controller {
	...
	public function index()
	{
		...
		else if($p_func == "LOGIN_WITH_INSTITUTION" || $p_func == "LOGIN_WITH_INSTITUTION_MFA")
		{
			$this->load->model('Finicity_Model');
			...
			{	
				...
				if(!empty($arInstitutionDetails))
				{
					...
					if($p_func == "LOGIN_WITH_INSTITUTION_MFA")
					{
					...
					if($response)
					{
						if($is_activation || $is_get_statement)
						{
							...
							else if($this->Finicity_Model->isGetStatementMFA)
							{
								...
								<form name="frmBankLoginMFA" id="frmBankLoginMFA" action="" method="post">
									<h6 class="color-blue">Fetching your account details, please answer once more...</h6>
									...
									<div class="form-group">
										...
										<p class="pull-right"><label>Time Left:</label> <span id="seconds">119</span> seconds</p>
									</div>
									...
									<input type="hidden" name="arLogin[MFA-Session]" value="<?=$this->Finicity_Model->szGetStatementMFASession?>">
						...
						else
						{
							...
							else if($this->Finicity_Model->isLoginMFA)
							{
								echo "MFA|||{$arInstitutionDetails[0]['szName']}|||";?>				
								<form name="frmBankLoginMFA" id="frmBankLoginMFA" action="" method="post">
									<h6 class="color-blue">Activating your account, please answer once more...</h6>
									<?php foreach($this->Finicity_Model->arMFA['questions'] as $i=>$question){?>
									<?php if(!empty($question['choice'])){?>
									<div class="form-group">
										<label><?=trim($question['text'])?></label>
									...
									<input type="hidden" name="arLogin[questions][<?=$i?>][text]" value="<?=sanitize_post_field_value($question['text'])?>">
									<input type="hidden" name="arLogin[id]" value="<?=$idInstitute?>">
									<input type="hidden" name="arLogin[MFA-Session]" value="<?=$this->Finicity_Model->szLoginMFASession?>">
									<input type="hidden" name="arLogin[activation-mfa]" value="1">
									<?php if(!empty($this->Finicity_Model->arLoginResponse['account'])){$isCheckingFound = $isSavingFound = false;foreach($this->Finicity_Model->arLoginResponse['account'] as $k=>$account){ if((!$isCheckingFound && trim($account['type']) == "checking") || (!$isSavingFound && trim($account['type']) == "savings")){?>							
					...
					else if($this->Finicity_Model->isLoginMFA)
					{
						echo "MFA|||{$arInstitutionDetails[0]['szName']}|||";?>				
						<form name="frmBankLoginMFA" id="frmBankLoginMFA" action="" method="post">
							<?php if($is_activation){?><h6 class="color-blue">Activating your account, please answer once more...</h6><?php } else {?><h6 class="color-blue">Adding your accounts, please answer...</h6><?php }?>
							<?php foreach($this->Finicity_Model->arMFA['questions'] as $i=>$question){?>
							<?php if(!empty($question['choice'])){?>
							<div class="form-group">
								<label><?=trim($question['text'])?></label>

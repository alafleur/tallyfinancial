...
class User_Model extends Database_Model
{
	...
	{	
		...
		if(!$byMobilePhone && !$szEmail && !empty($user_session['login']))
		{
	...
	function updateCustomerInstitution($institution_id, $account_id, $account_number, $statement_file)
	{
		if((int)$institution_id > 0 && (int)$account_id > 0 && trim($account_number) != '' && trim($statement_file) != '')
		{			
			$query = "
				UPDATE
					" . __DBC_SCHEMATA_USERS__ . "
				SET
					...
					idFinicityAccount = " . (int)$account_id . ",
					szFinicityAccountNumber = '" . $this->sql_real_escape_string(trim($account_number)) . "',
					szFinicityStatementVerificationFile = '" . $this->sql_real_escape_string(trim($statement_file)) . "',
					iSignupStep = 3
				WHERE
					id = " . (int)$this->id . "
			";

...
<div class="tn-search-form">
	<form id="frmSearchTransactions" name="frmSearchTransactions" method="post" action="<?=__BASE_ADMIN_URL__?>/users/managebalance/<?=$idUser?>" class="form-horizontal">
		...
		<h3>Approve user $<?=format_number($arg4, true)?> withdraw request</h3>
		<?php }?>
		<div class="form-group<?=(!empty($obj->User_Model->arErrorMessages['p_amount']) ? ' has-error' : '')?>">
			...
			<div class="col-sm-6 color-blue">
				<i class="fa fa-info-circle"></i> Total available balance: $<?=format_number($fTallyBalance, true)?>
			</div>
...
<div class="dash-box">
	...
	<ul>
		...
		<li<?=(!$show_more && ($i+1) == $iLimit ? ' class="last"' : '')?>><strong>$ <?=format_number($transaction['fAmount'], true)?> transferred</strong> on <?=($transaction['iType'] == 2 ? date("F j, Y", strtotime($transaction['dtApprovedOn'])) : date("F j, Y", strtotime($transaction['dtCreatedOn'])))?> <?=($transaction['iType'] == 2 ? " (Requested on " . date("F j, Y", strtotime($transaction['dtCreatedOn'])) . ")" : "")?></li>
		<?php } if($show_more){?>

		...
		if(!$is_error)
		{
			...
			{				
				if($obj->User_Model->loadCustomer($idUser))
				{
					...
					$replace_ary['CHEQUINGBALANCE'] = format_number($fChequingbalance, true);
					$replace_ary['TOTALSAVINGS'] = format_number($fTotalSaving, true);
					$replace_ary['TALLYBALANCE'] = format_number($fTallyBalance, true);
					$replace_ary['EXTERNALBALANCE'] = format_number($fTotalTransfers, true);
					$replace_ary['MOSTRECENT'] = format_number($fMostRecentTransaction, true);

<?php
require_once(__APP_PATH_ADMIN__ . "/handler.php");
...
if(!empty($_POST['arSearch']))
{
	...
	if(!empty($dtSearch))
	{
		...
		{
			$dtSearch = date("l jS M Y", strtotime($dtSearch));
			$arFiles = glob( __APP_PATH__ . "/output/biweekly/*csv");
		...
		{
			$kObj->addError("dtSearch", "Please select a past date of Monday or Thursday");
		}
	}
	else
	{
	...
	<form id="frmDownload" name="frmDownload" method="post" action="<?=__BASE_ADMIN_URL__?>/download/files" class="form-horizontal">
		<div class="row">
			<div class="col-sm-12 col-md-8">
				<div class="form-group<?=(!empty($kObj->arErrorMessages['dtSearch']) ? ' has-error' : '')?>">
					...
					<?=(!empty($kObj->arErrorMessages['dtSearch']) ? '<span class="help-block col-sm-offset-3 pull-left"><i class="fa fa-times-circle"></i> '.$kObj->arErrorMessages['dtSearch'].'</span>' : '')?>
				</div>

...
if(isset($_POST['p_withdraw']))
{
	...
	if($obj->User_Model->validateInput($_POST['p_withdraw'], __VLD_CASE_NUMERIC__, "p_withdraw", "Withdraw amount", 1.00, ($fTotalAvalable > 0 ? $fTotalAvalable : false), true))
	{
		...
		else
		{
			...
			{
				$done = "Your withdraw request successfully submitted. Admin will process your request as soon as possible.";
				
				// send message to admin
				sendMessege(__ADMIN_MOBILE_NUMBER__, "User {$obj->User_Model->szFirstName} {$obj->User_Model->szLastName}({$obj->User_Model->szEmail}) has requested for withdrawal of $" . format_number($fTransferAmount) . " from Tally savings. Please check and approve the request.");
				
				// send email
				$message = "
				User {$obj->User_Model->szFirstName} {$obj->User_Model->szLastName}({$obj->User_Model->szEmail}) has requested for withdrawal of $" . format_number($fTransferAmount) . " from Tally savings.<br><br>
									...
									<div class="col-sm-9">
										<div class="form-group<?=(!empty($obj->User_Model->arErrorMessages['p_withdraw']) ? ' has-error' : '')?>">
											...
											<div class="col-sm-7">
												...
												<span class="pull-left color-blue"><i class="fa fa-info-circle"></i> Total available: <?=format_number($fTallyBalance)?></span>
